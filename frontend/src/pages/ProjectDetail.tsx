      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="secondary" size="sm" onClick={async () => {
            try {
              await projectsApi.exportPackage(projectId!, project.name)
              toast.success('Package downloaded — Word cover pages + all records + README')
            } catch (err: any) {
              toast.error(err?.response?.data?.detail || 'Export failed — no records in project')
            }
          }}>
            <Download className="w-3.5 h-3.5" /> Download Full Package
          </Button>
        </div>
      </div>
