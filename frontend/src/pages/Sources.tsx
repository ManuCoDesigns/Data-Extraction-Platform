...
export function SourcesPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || (!form.schema_id && !form.no_schema) || !targetProjectId) return
    setCreating(true)
    try {
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="New Source" description="Track a new dataset through extraction, review, and approval." size="md">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <label style={{display:'flex',alignItems:'center',gap:8,margin:'4px 0'}}>
            <input type="checkbox" checked={form.no_schema}
              onChange={e => setForm(f => ({ ...f, no_schema: e.target.checked, schema_id: e.target.checked ? '' : f.schema_id }))} />
            <span style={{fontSize:13,color:'#64748b'}}>This source has no fixed schema (flexible extraction only)</span>
          </label>

          {!form.no_schema && (
            <>
              <Select label="Schema" value={form.schema_id} onChange={e => setForm(f => ({ ...f, schema_id: e.target.value }))} required={!form.no_schema} disabled={isGlobal && !form.project_id}>
                <option value="">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button type="submit" loading={creating} disabled={!form.name.trim() || (!form.schema_id && !form.no_schema)}>
